...
class AddToCartView(CartMixin, View):
    ...
    def post(self, request, slug):
        ...
        if size_id:
            ...
            product_size = product.product_sizes.filter(stock__gt=0)
            if not product_size:
                return JsonResponse({
    ...
    def post(self, request, item_id):
        ...
        if quantity == 0:
            ...
            if quantity > cart_item.product_size.stock:
                return JsonResponse({
                    'error': f'Only {cart_item.product_size.stock} items'
                }, status=400)
...
class CartSummaryView(CartMixin, View):
    def get(self, request):
        ...
        return TemplateResponse(request, 'cart/cart_summaty.html', context)
